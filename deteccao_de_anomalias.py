...
def aplicar_modelos(nome_dataset, features, labels=None):
    ...
    if labels is not None:
        ...
        print("\n[Individual Models]")
        evaluate_model(labels, iso_pred, f"Isolation Forest - {nome_dataset}")
        evaluate_model(labels, lof_pred, f"Local Outlier Factor - {nome_dataset}")
        if labels is not None and auto_pred is not None:
            evaluate_model(labels, auto_pred, f"Autoencoder - {nome_dataset}")
    else:
